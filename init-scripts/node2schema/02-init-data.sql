USE games_database;

INSERT INTO games (game_id, name, required_age, price, metacritic_score, achievements) VALUES
(1, 'Half-Life 2', 18, 9.99, 96, 100),
(2, 'Portal 2', 12, 19.99, 95, 51),
(3, 'Counter-Strike: Global Offensive', 18, 0.00, 88, 167),
(4, 'Dota 2', 0, 0.00, 90, 45),
(5, 'Team Fortress 2', 12, 0.00, 92, 520),
(6, 'Apex Legends', 16, 0.00, 88, 120),
(7, 'Valorant', 16, 0.00, 90, 200),
(8, 'The Witcher 3: Wild Hunt', 18, 39.99, 93, 78),
(9, 'Minecraft', 7, 26.99, 93, 113),
(10, 'Red Dead Redemption 2', 18, 59.99, 93, 45);

DROP TABLE IF EXISTS games_for_everyone;
CREATE TABLE games_for_everyone AS
SELECT * FROM games
WHERE required_age <= 17;